...
<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="table-container">
								<div class="t-header">
                                <h4>List Booking Invoices / Pay Off</h4>
								...
								<div class="table-responsive">
									<table width="100%" class="table custom-tablex" id="basicExamplex">
                                        ...
                                        <?php
										$x=1;
									$select_stmt=$db->prepare("SELECT * FROM payments WHERE sts_pay='1' ORDER BY id DESC");	//sql select query
									$select_stmt->execute();
 ...
 $select_cont=$db->prepare("SELECT
   ...
   $row_cont=$select_cont->fetch(PDO::FETCH_ASSOC);
                    ...
                    <?php } ?>
											  ...
											  <td align="center">
                                              <?php if ($row['approve']=="3" && $row['sts_end']=="1") { ?>
                                              <a href="javascript:void(0);" class="openPopup" data-href="modul/adm/form_payment.php?id_=<?php echo $row['id']; ?>" id="xxx">
                    <img src="img/pay.png" width="41" height="40" />
                    </a>
                    <?php } ?>
                                              </td>
						...
						<strong><?php echo $row_cont['first_name'].' '.$row_cont['last_name']; ?></strong><br> 
											  <?php echo $row_cont['nm_negara'].','.$row_cont['nm_state'].','.$row_cont['nm_city']; ?><br>
											  <?php echo $row_cont['email'].', '.$row_cont['phone_number']; ?></td>
                                              <?php
											  $select_payx=$db->prepare("SELECT * FROM pay_instructor WHERE id_payment='$row[id]'");	//sql select query
									$select_payx->execute();
									$row_payx=$select_payx->fetch(PDO::FETCH_ASSOC);
											  ?>
												<td><?php echo $row['payment_amount']; ?></td>
												<td><?php echo $row_payx['fee']; ?></td>
												<td><?php echo $row_payx['nominal']; ?></td>
												<td>
												<?php echo $data[0]; ?></td>
											</tr>
                                            <?php $x+=1; ?>
